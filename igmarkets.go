...
func (ig *IGMarkets) ReadLiveStream(ctx context.Context, temp []*Stock_local, ticker_ig string, ticker string) {
	for {
		prices, error := ig.GetPrice(ctx, ticker_ig)
		if error == nil {
			temp_AAPL := Stock_local{Ticker: ticker, ClosePrice: prices.Prices[0].ClosePrice.Ask, Date: time.DateOnly}
